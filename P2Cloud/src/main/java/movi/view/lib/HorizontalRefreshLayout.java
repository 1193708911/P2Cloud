...
import movi.ui.view.lib.RefreshHeader;
...
public class HorizontalRefreshLayout extends FrameLayout {
    ...
    private float leftHeadWidth = 100;
...
//                    Log.v("AAA", child1.getTranslationX() + "");
                    if (leftRefreshHeader != null) {
                        ...
                        if (child1.getTranslationX() >= (leftHeadWidth - dp2px(context, 16)) && refreshState != REFRESH_STATE_READYTORELEASE) {
                            ...
                            LogUtil.d("--------onReadyToRelease" + "当前已执行");
                        }
                    }
                }
                return true;
...
//                    Log.v("AAA", child1.getTranslationX() + "");
    ...
    public void onRefreshComplete() {
        ...
        smoothRelease();
        if (leftRefreshHeader != null)
            leftRefreshHeader.onStart(START, leftHead); //reset view style
        if (rightRefreshHeader != null)
            rightRefreshHeader.onStart(END, rightHead); //reset view style
    }
