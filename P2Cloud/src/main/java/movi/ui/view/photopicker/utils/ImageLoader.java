package movi.ui.view.photopicker.utils;
...
import movi.ui.view.io.DiskLruCache;
...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = movi.view.photopicker.utils.OtherUtils.getDiskCacheDir(MainApplication.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(MainApplication.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
